...
export default function LandingPage() {
  ...
  const faqItems = [
    {
      ...
      a: 'Não. O Agendy funciona direto no navegador, tanto para você quanto para seus clientes. Basta acessar pelo celular ou computador.',
    },
    {
  ...
  return (
    <>
      <SEO
        title="Agendy - Agenda online para salões, barbearias e estúdios"
        description="Agenda online para salões, barbearias e estúdios. Seus clientes agendam em segundos, sem baixar app e sem criar conta."
      />
      <div className="min-h-screen bg-white">
        {/* Header */}
        <header className="sticky top-0 z-50 bg-white/95 backdrop-blur border-b border-gray-100">
          <div className="max-w-6xl mx-auto px-4 py-3 md:py-4 flex items-center justify-between gap-3">
            <Link to="/" className="text-lg md:text-xl font-bold text-violet-300 flex-shrink-0">
              Agendy
            </Link>

            <nav className="hidden md:flex items-center gap-6 text-sm font-medium text-gray-700">
              <button onClick={() => scrollToSection('como-funciona')} className="hover:text-violet-300 transition">
                Como funciona
              </button>
              <button onClick={() => scrollToSection('planos')} className="hover:text-violet-300 transition">
                Planos
              </button>
              <button onClick={() => scrollToSection('faq')} className="hover:text-violet-300 transition">
                Dúvidas
              </button>
            </nav>
            ...
            <div className="flex items-center gap-2 md:gap-3">
              <Link
                to="/login"
                className="hidden sm:inline text-sm font-medium text-gray-700 hover:text-violet-300 transition"
              >
                Entrar
              </Link>
              <Link
                to="/cadastro"
                className="bg-violet-300 text-white text-xs md:text-sm font-medium px-4 md:px-5 py-2 md:py-2.5 rounded-full hover:bg-violet-300 transition whitespace-nowrap"
              >
                Começar grátis
              </Link>
          ...
          {menuOpen && (
            <div className="md:hidden border-t border-gray-100 bg-white">
              <nav className="flex flex-col px-4 py-3 space-y-1 text-sm font-medium text-gray-700">
                <button onClick={() => scrollToSection('como-funciona')} className="text-left py-2.5 hover:text-violet-300 transition">
                  Como funciona
                </button>
                <button onClick={() => scrollToSection('planos')} className="text-left py-2.5 hover:text-violet-300 transition">
                  Planos
                </button>
                <button onClick={() => scrollToSection('faq')} className="text-left py-2.5 hover:text-violet-300 transition">
                  Dúvidas
                </button>
                <Link to="/login" className="py-2.5 hover:text-violet-300 transition">
                  Entrar
                </Link>
              </nav>
            </div>
        ...
        <section className="relative overflow-hidden">
          ...
          <div className="relative max-w-6xl mx-auto px-4 py-16 md:py-28 grid grid-cols-1 md:grid-cols-2 gap-10 md:gap-16 items-center">
            <div>
              <Reveal>
                <span className="inline-flex items-center gap-1.5 bg-violet-100 text-violet-300 text-xs md:text-sm font-medium px-3 py-1.5 rounded-full mb-5">
              ...
              <Reveal delay={100}>
                <h1 className="text-4xl md:text-6xl font-bold text-gray-900 leading-[1.1] tracking-tight mb-5">
                  ...
                  agendam.
                  <br />
                  <span className="text-violet-300">Você só atende.</span>
                </h1>
              </Reveal>
              ...
              <Reveal delay={300}>
                <div className="flex flex-col sm:flex-row sm:items-center gap-3 sm:gap-5">
                  <Link
                    to="/cadastro"
                    className="bg-violet-300 text-white px-7 py-3.5 rounded-full font-semibold hover:bg-violet-300 transition text-center shadow-lg shadow-violet-300/20"
                  >
                    Criar minha agenda
                  </Link>
                    ...
                    <Reveal key={plan.key} delay={(index + 1) * 100} className="h-full">
                      <div
                        ...
                        className={`cursor-pointer w-full bg-white rounded-3xl p-6 md:p-8 flex flex-col h-full relative transition-all duration-200 ${isSelected
                          ? 'border-2 border-violet-300 shadow-xl shadow-violet-300/10'
                          : 'border border-gray-200 hover:border-gray-300'
                          }`}
                      >
                        {plan.isPopular && (
                          <span className="absolute -top-3 left-1/2 -translate-x-1/2 bg-violet-300 text-white text-xs font-semibold px-3 py-1 rounded-full whitespace-nowrap">
                            Mais popular
                          </span>
                        )}
                        ...
                        <Link
                          ...
                          className={`block text-center py-3 rounded-full font-semibold transition ${isSelected
                            ? 'bg-violet-300 text-white hover:bg-violet-300'
                            : 'border border-violet-300 text-violet-300 hover:bg-violet-50'
                            }`}
                        >
                          {plan.ctaLabel}
                        </Link>
                ...
                <Reveal key={index} delay={(index + 1) * 100}>
                  <div className="border border-gray-200 rounded-2xl overflow-hidden bg-white">
                    ...
                    >
                      ...
                      <span className="text-violet-300 text-xl leading-none flex-shrink-0">
                        {openFaq === index ? '−' : '+'}
                      </span>
                    </button>
                    {openFaq === index && (
        ...
        {/* CTA final */}
        <section className="py-16 md:py-24 bg-violet-300 relative overflow-hidden">
          <div
            className="absolute -top-24 -right-24 w-96 h-96 rounded-full bg-violet-300/40 blur-3xl"
            aria-hidden="true"
          />
          <div
            className="absolute -bottom-24 -left-24 w-80 h-80 rounded-full bg-violet-300/30 blur-3xl"
            aria-hidden="true"
          />
          ...
          <div className="relative max-w-3xl mx-auto px-4 text-center">
            ...
            <Reveal delay={200}>
              <Link
                to="/cadastro"
                className="inline-block bg-white text-violet-300 px-8 py-3.5 rounded-full font-semibold hover:bg-violet-50 transition shadow-xl shadow-violet-300/20"
              >
          ...
          <div className="max-w-6xl mx-auto px-4">
            <div className="grid grid-cols-1 md:grid-cols-4 gap-8 mb-10">
              <div className="md:col-span-2">
                <p className="text-white text-lg md:text-xl font-bold mb-3">Agendy</p>
            ...
            <div className="border-t border-gray-800 pt-6 flex flex-col md:flex-row items-center justify-between gap-3">
              <p className="text-xs text-center md:text-left">
                © {new Date().getFullYear()} Agendy. Todos os direitos reservados.
              </p>
