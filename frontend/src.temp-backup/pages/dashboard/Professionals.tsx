    ...
    <div className="p-4 md:p-8">
      ...
      <div className="flex flex-col gap-4 mb-6 animate-fade-in-up md:flex-row md:items-center md:justify-between">
        ...
        {limitReached ? (
          ...
          <Link
            ...
            className="inline-flex items-center justify-center gap-2 bg-violet-300 text-white px-4 py-2.5 rounded-lg font-medium hover:bg-violet-300 transition self-start md:self-auto w-full md:w-auto"
          >
      ...
      {plan && maxProfessionals !== null && (
        ...
        >
          {limitReached ? (
            ...
            <Users className="w-5 h-5 text-violet-300 flex-shrink-0 mt-0.5 md:mt-0" />
          )}
          <div className="flex-1 min-w-0">
            <p className={`text-sm font-medium ${limitReached ? 'text-yellow-900' : 'text-violet-300'}`}>
              {limitReached
                ? `Você atingiu o limite do plano ${plan.planName}`
                : `Plano ${plan.planName}`}
            </p>
            <p className={`text-xs ${limitReached ? 'text-yellow-800' : 'text-violet-300'}`}>
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-8 md:p-12 text-center animate-fade-in-up">
          ...
          <Link
            to="/admin/professionals/new"
            className="inline-flex items-center gap-2 bg-violet-300 text-white px-4 py-2 rounded-lg text-sm font-medium hover:bg-violet-300"
          >
            <Plus className="w-4 h-4" />
            Criar profissional
          </Link>
                    ...
                    <tr key={p.id} className="hover:bg-gray-50 transition">
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="flex items-center gap-3">
                          <div className="w-9 h-9 rounded-full bg-violet-100 flex items-center justify-center text-violet-300 text-sm font-semibold flex-shrink-0">
            ...
            {professionals.map((p, index) => (
              ...
              >
                ...
                <div className="flex items-start gap-3 mb-3">
                  <div className="w-11 h-11 rounded-full bg-violet-100 flex items-center justify-center text-violet-300 text-base font-semibold flex-shrink-0">
                    {p.name?.[0]?.toUpperCase() || 'P'}
                  </div>
                ...
                <div className="flex items-center gap-2 pt-3 border-t border-gray-100">
                  ...
                  <Link
                    to={`/admin/professionals/${p.id}/edit`}
                    className="flex-1 inline-flex items-center justify-center gap-1.5 py-2 rounded-lg text-sm font-medium text-violet-300 bg-violet-50 hover:bg-violet-100 transition"
                  >
                    <Pencil className="w-4 h-4" />
                    Editar
                  </Link>
